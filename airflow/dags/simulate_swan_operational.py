...
def create_dag(dag_id, parameters):
    ...
    dag = DAG(
        ...
        description='Operational SWAN simulation.',
        ...
        user_defined_macros={'filesystem': '/opt/airflow/filesystem',
                             ...
                             'model': 'swan/' + parameters["simulation_id"],
                             'docker': parameters["docker"],
                             'simulation_folder_prefix': format_simulation_directory(parameters["docker"], "swan"),
                             ...
                             'api_server_folder': "/nfsmount/filesystem/media/simulations/swan/results/{}".format(
    ...
    run_simulation = BashOperator(
        ...
        bash_command='docker run '
                     '-v {{ FILESYSTEM }}/git/{{ simulation_repo_name }}/runs/{{ simulation_folder_prefix }}_{{ id }}_{{ start(ds) }}_{{ end(ds) }}:/home/swan '
                     '--rm '
                     '{{ docker }} '
...
with open('dags/simulate_swan_operational.json') as f:
    simulations = json.load(f)

for simulation in simulations:
    dag_id = "simulate_swan_operational_" + simulation["simulation_id"]
