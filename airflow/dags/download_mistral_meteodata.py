...
dag = DAG(
    'download_mistral_meteodata',
    default_args=default_args,
    description='Download Meteodata from MISTRAL.',
...
download = BashOperator(
    ...
    bash_command="mkdir -p {{ params.filesystem }}; cd {{ params.git_dir }}; python src/main.py -s {{ params.source }} "
                 "-f {{ params.filesystem }} -u {{ params.MISTRAL_USER }} -p {{ params.MISTRAL_PASSWORD }}",
    params={'git_dir': '/opt/airflow/filesystem/git/alplakes-externaldata',
            'source': 'mistral_meteodata',
            'filesystem': '/opt/airflow/filesystem/media',
            'MISTRAL_USER': Variable.get("MISTRAL_USER"),
            'MISTRAL_PASSWORD': Variable.get("MISTRAL_PASSWORD")},
    on_failure_callback=report_failure,
    dag=dag,
)
