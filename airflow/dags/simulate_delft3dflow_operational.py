...
def create_dag(dag_id, parameters):
    ...
    prepare_simulation_files = BashOperator(
        ...
        bash_command="mkdir -p {{ filesystem }}/git;"
                     ...
                     "python src/main.py -m {{ model }} -d {{ docker }} {{ restart_file }} -t {{ today(ds) }} -s {{ start(ds) }} -e {{ end(ds) }} -b {{ bucket }} -a {{ api }} -u",
    ...
    prepare_simulation_files >> run_simulation >> postprocess_simulation_output >> send_results >> remove_results >> cache_data
